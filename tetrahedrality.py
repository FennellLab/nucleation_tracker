...
with open (inputFileName, 'r') as inFile:
	...
	for i, line in enumerate(inFile):
		if str(line[10:15]).strip() in ["OW", "O"]:
			...
			outputFileName.write("COMPD    %s\n" %(inputFileName))
			outputFileName.write("REMARK  %6.2f  %6.2f\n" %(avg_tetrahedrality, stdev_tetrahedrality))
			outputFileName.write("AUTHOR   PDB file generated by nucleation_tracker program\n")
			...
			tetrahedrality = []
			deviation = 0
			nearestNeighbor_dist = [[] for _ in range(int(nMols))]
			nearestNeighbor_index = [[] for _ in range(int(nMols))]
